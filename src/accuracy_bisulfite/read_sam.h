...
void Read_SAM_Results(const char* file_name, vector<CMAPPINGResult>& res,
		const string& mapper, const bool& paired) {
	FILE * fin = fopen(file_name, "r");
	if (!fin) {
		cerr << "cannot open input file " << file_name << endl;
	}

	string qname, rname, cigar, rnext, seq, qual, NM, XO, XS, MD, XM, XR, XG;
	uint32_t flag, mapq, pnext, tlen, pos;
	int mismatch = 100;
	//char strand = '+';

	string qname2, rname2, cigar2, rnext2, seq2, qual2, NM2, XO2, XS2, MD2, XM2,
			XR2, XG2;
	...
	while (fgets(cline, MAX_LINE_LENGTH, fin)) {
		...
		pos = 0;
		pos2 = 0;
		if (mapper == "-bsmap" || mapper == "bsmap" || mapper == "-bwameth"
				|| mapper == "bwameth") {
			iss >> qname >> flag >> rname >> pos >> mapq >> cigar >> rnext
					>> pnext >> tlen >> seq >> qual >> NM >> MD;
		} else if (mapper == "-bismark" || mapper == "bismark") {
			iss >> qname >> flag >> rname >> pos >> mapq >> cigar >> rnext
					>> pnext >> tlen >> seq >> qual >> NM >> MD >> XM >> XR
					>> XG;
		} else if (mapper == "-walt" || mapper == "walt") {
			iss >> qname >> flag >> rname >> pos >> mapq >> cigar >> rnext
					>> pnext >> tlen >> seq >> qual >> NM;
		} else if (mapper == "-bsseeker" || mapper == "bsseeker") {
			iss >> qname >> flag >> rname >> pos >> mapq >> cigar >> rnext
					>> pnext >> tlen >> seq >> qual >> XO >> XS >> NM;
		} else if (mapper == "-brat_nova" || mapper == "brat_nova") {
			iss >> qname >> flag >> rname >> pos >> mapq >> cigar >> rnext
					>> pnext >> tlen >> seq >> qual;
		} else {
		...
		if (flag_lab.paired_mapped) {
			...
			if (mapper == "-bsmap" || mapper == "bsmap" || mapper == "-bwameth"
					|| mapper == "bwameth") {
				iss >> qname2 >> flag2 >> rname2 >> pos2 >> mapq2 >> cigar2
						>> rnext2 >> pnext2 >> tlen2 >> seq2 >> qual2 >> NM2
						>> MD2;
			} else if (mapper == "-bismark" || mapper == "bismark") {
				iss >> qname2 >> flag2 >> rname2 >> pos2 >> mapq2 >> cigar2
						>> rnext2 >> pnext2 >> tlen2 >> seq2 >> qual2 >> NM2
				...
				iss >> qname2 >> flag2 >> rname2 >> pos2 >> mapq2 >> cigar2
						>> rnext2 >> pnext2 >> tlen2 >> seq2 >> qual2 >> XO2
						>> XS2 >> NM2;
			} else if (mapper == "-brat_nova" || mapper == "brat_nova") {
				iss >> qname >> flag >> rname >> pos >> mapq >> cigar >> rnext
						>> pnext >> tlen >> seq >> qual;
